...
# In this section, you will load a Dataset object, but first you must transform the dataset. Use the Compose function to
# perform the following transforms:.
#  - use the transforms object Resize to resize the image.
#  - use the transforms object ToTensor to concert the image to a tensor.
...
# Create two dataset objects for the Fashion MNIST dataset. One for training data called dataset_train and one for
# validation data dataset_val.
...
train_loader = torch.utils.data.DataLoader(dataset=dataset_train, batch_size=100, **kwargs_loader)
test_loader = torch.utils.data.DataLoader(dataset=dataset_val, batch_size=100, **kwargs_loader)


# #### Build Convolutional Neural Network (with and without Batch Normalization) Classes and the Training Function

# Create a Convolutional Neural Network class using ONE of the following constructors. Train the network using the
# provided code then provide a screenshot of your training cost and accuracy with your validation data.
...
model = CNN(out_1=16, out_2=16*2, out_dim=out4[0], n_classes=len(dataset_train.classes),
            ks_conv=ks_conv, ks_pool=ks_pool,
            stride_conv=stride_conv, stride_pool=stride_pool, padding=padding).to(device)
model_batch = CNN_batch(out_1=16, out_2=16*2, out_dim=out4[0], n_classes=len(dataset_train.classes),
            ks_conv=ks_conv, ks_pool=ks_pool,
            stride_conv=stride_conv, stride_pool=stride_pool, padding=padding).to(device)
...
t0 = time()
loss_list, accuracy_list = train(train_loader, test_loader,
...
t0 = time()
loss_list_batch, accuracy_list_batch = train(train_loader, test_loader,
...
print('\nThe maximum accuracy value reached on the validation set without batch normalization is:', accuracy_list[-1], '%')
print('While that achieved with batch normalization is:', accuracy_list_batch[-1], '%')
...
test_set = torch.utils.data.DataLoader(dataset=dataset_val, batch_size=1, **kwargs_loader)
model_batch.eval()
y_hat = []
y_hat_batch = []
model.eval()

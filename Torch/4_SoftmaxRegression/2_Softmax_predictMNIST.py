...
# object in the argument transform. Then print the training dataset.
...
# object in the argument transform. Then print the validating dataset.
...
def plot_misclass(modello):
    Softmax_fn = nn.Softmax(dim=-1)
    count = 0
    for x, y in validation_dataset:
        z = modello(x.to(device).reshape(-1, 28 * 28))
...
# We Plot the first five correctly classified samples and the probability of that class, we see the probability is much
# larger.
def plot_correct(modello):
    Softmax_fn = nn.Softmax(dim=-1)
    count = 0
    for x, y in validation_dataset:
        z = modello(x.to(device).reshape(-1, 28 * 28))
        _, yhat = torch.max(z, 1)
        if yhat == y:
            print("\nCorrectly classified sample number", count, "has actual label", y,
                  "\nbut was attributed to class", yhat.item(), "with probability", torch.max(Softmax_fn(z)).item())
            show_data((x, y))
            count += 1
        if count >= 5:

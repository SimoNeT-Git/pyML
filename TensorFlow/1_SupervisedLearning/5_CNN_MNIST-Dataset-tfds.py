...
# -----------------
# #### Load MNIST dataset
print('\n\nFirst of all we must load the MNIST datset.')
...
# to read more about it at: http://yann.lecun.com/exdb/mnist/
...
(mnist_train, mnist_test), info = tfds.load(name='mnist', split=['train', 'test'],
                                            with_info=True, as_supervised=True)
print('\nInformation on the MNIST dataset:')
print(info)
# mnist_test, info_test = tfds.load(name='mnist', split=tfds.Split.TEST, with_info=True)
# print('\nInformation on the MNIST test set:')
# print(info_test)
...
# Define one hot encode for 10 MNIST classes
def my_one_hot(feature, label):
    return feature, tf.one_hot(label, depth=10)


# Split the dataset in training set and test set and encode labels with one_hot representation
mnist_train, mnist_test = mnist_train.map(my_one_hot), mnist_test.map(my_one_hot)

# Lets batch our training set (divide it in batches of 50 samples each)
batch_size = 50
batch_train = mnist_train.batch(batch_size)
batch_test = mnist_test.batch(batch_size)

# Note: the one-hot representation, in contrast to Binary representation, means that the data labels will be presented
...
# We could also import data using the input_data.py script and particularly using the following lines of code:
# mnist = read_data_sets("Data/MNIST_data/", one_hot=True)
# mnist_train, mnist_test = mnist.train, mnist.test
# The whole mnist dataset woul be imported and saved in ./Data/MNIST_data/. It is divided as follow:
...
# First of all we have to make the training set iterable
train_iterator = sess.run(tf.compat.v1.data.make_initializable_iterator(batch_train))
next_train_sample = train_iterator.get_next()
...
for i in range(1000):
    ...
    batch_train_x, batch_train_y = sess.run(next_train_sample)
    sess.run(optimizer, feed_dict={x: batch_train_x, y_: batch_train_y})

# ## Test
# First lets make an iterator for the test set
test_iterator = tf.compat.v1.data.make_initializable_iterator(batch_test)
next_test_sample = test_iterator.get_next()
# Define prediction
...
n_batches = info.splits["test"].num_examples // batch_size
cumulative_accuracy = 0.0
for i in range(n_batches):
    sess.run(test_iterator.initializer)
    batch_test_x, batch_test_y = sess.run(next_test_sample)
    cumulative_accuracy += accuracy.eval(feed_dict={x: batch_test_x, y_: batch_test_y})
...
# Is the final result good? Let's check the best algorithm available out there (10th June 2016):
...
# ## Flattening the output of the second layer
...
# ## Train the network
# First lets make an iterator for the training set
train_iterator = tf.compat.v1.data.make_one_shot_iterator(batch_train)
next_train_sample = train_iterator.get_next()
...
def train(n_epochs):
    ...
    for i in range(n_epochs):
        ...
        if i % 100 == 0:
            train_accuracy = accuracy.eval(feed_dict={x: batch_train_x, y_: batch_train_y, keep_prob: 1.0})
            print("At step %d training accuracy is %g" % (i, float(train_accuracy)))

        sess.run(optimizer, feed_dict={x: batch_train_x, y_: batch_train_y, keep_prob: 0.5})
...
# First lets make an iterator for the test set
test_iterator = tf.compat.v1.data.make_one_shot_iterator(batch_test)
next_test_sample = test_iterator.get_next()
...
for i in range(n_batches):
    sess.run(test_iterator.initializer)
    batch_test_x, batch_test_y = sess.run(next_test_sample)
    cumulative_accuracy += accuracy.eval(feed_dict={x: batch_test_x, y_: batch_test_y, keep_prob: 1.0})
acc = cumulative_accuracy / n_batches * 100
...
sampleimage, = mnist_test.take(1)
sampleimage = sampleimage[0]
